...
module ``07: The Good Kind of Discrimination`` = 
    ...
    let ``02 Creating & pattern-matching a discriminated union`` () = 
        let randomOpinion degree =
            ...
            | BSc _ -> "!!SCIENCE!!"
            | BPharm -> "Meh, it's OK."
            | BCom (Management, _) -> "Money, money, money."  //| FILL_ME_IN -> "Money, money, money."
            | BCom (Linguistics, Management) -> "Money, money, money."
            | BA (Linguistics, Philosophy) -> "A thinker, eh?"        //| FILL_ME_IN -> "A thinker, eh?"
        randomOpinion (BSc (ComputerScience, Mathematics)) |> should equal "Good choice!" //randomOpinion __ |> should equal "Good choice!"
        randomOpinion (BSc (Mathematics, Philosophy)) |> should equal "!!SCIENCE!!"  //randomOpinion __ |> should equal "!!SCIENCE!!"
    ...
    let ``03 We can create a discriminated union using named fields`` () =
        let someDegree = BSc (second = Mathematics, first = ComputerScience)      // let someDegree = BSc (second = __, first = __)        
    ...
    let ``04 Pattern-matching using named fields`` () =
        let result =
            ...
            | BSc (_, ComputerScience)  -> "correct" // <-- USE a pattern-match with named fields!   // | FILL_ME_IN -> "correct"
            | _ -> "nope"
