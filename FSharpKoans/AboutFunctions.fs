...
module ``03: Putting the Function into Functional Programming`` = 
    ...
    let ``15 A function is executed when it is called, NOT when it is defined or referenced (Part 1).`` () =
        let f a =
            ...
            a + 2 
        f |> should be ofType<int -> int>   //___ |> should be ofType<int -> int>
    ...
    let ``17 Two names can be bound to the same value`` () =
        let f x = x + 2
        let y = f
        y 20 |> should equal 22    //y 20 |> should equal ___
    ...
    let ``18 Shadowing and functions`` () =
        ...
        let f () = a + 10
        let a = 99
        a |> should equal 99  //a |> should equal __
        f () |> should equal 35    //f () |> should equal __
    ...
    let ``19 Nesting functions`` () =
        let hailstone x =
            ...
            let addOne x = x + 1
            addOne (triple x)
        hailstone 5 |> should equal 16 //hailstone 5 |> should equal __

    [<Test>]
    let ``20 Functions have types`` () =
        let a x y = x + "cabbage" + y
        let b r = 50.0 / r
        a |> should be ofType<string -> string -> string> //a |> should be ofType<FILL_ME_IN>
    ...
    *)
        ...
        let square v = v * v
        somefunc 3 square |> should equal 12  //somefunc 3 square |> should equal __
        somefunc 3 ((*) 7) |> should equal 24 //somefunc 3 ((*) 7) |> should equal __
...
*)
    ...
    let ``23 |>, the 'pipe' operator`` () =
        ...
        3 |> double |> add5 |> should equal 11 //3 |> double |> add5 |> should equal __
    ...
    *)
    ...
    let ``26 >>, the 'compose' operator`` () =
        ...
        let l = j >> i
        i 3 |> should equal 16  //i 3 |> should equal __
        j 3 |> should equal 11    //j 3 |> should equal __
        k 3 |> should equal 17
        l 3 |> should equal 32    //check
    ...
    let ``28 Unit is used when there is no return value for a function``() = 
        ...
        //sendData "some data to send..." |> should equal ()
    ...
    *)
    ...
    let ``30 Unit is often used to defer code execution`` () =
        ...
        deferred |> should be ofType<unit -> int>   //deferred |> should be ofType<FILL_ME_IN>
        divideBy10 850 |> should be ofType<unit -> int>   //deferred |> should be ofType<FILL_ME_IN>
    ...
    *)
    ...
    let ``31 The 'ignore' function is used to map anything to 'unit'`` () =
        ...
        let log x =
            ...
            printfn "%A" x
            x // return x
        log 5 |> should equal 5  //log 5 |> should equal __
        ignore (log "blorp") |> should equal ()    //ignore (log "blorp") |> should equal __
        log 19.66 |> ignore |> should equal ()   //log 19.66 |> ignore |> should equal __
    ...
    let ``32 Partially specifying arguments (Part 1).`` () =
        ...
        let kittehs = f "cat"    //let kittehs = __ "cat"
        kittehs "nyan" |> should equal "cat says nyan"  // __ "nyan" |> should equal "cat says nyan"
    ...
    let ``33 Partially specifying arguments (Part 2).`` () =
        ...
        let f animal noise = animal + " says " + noise
        let howl k = f k "slash/crunch/snap"       //let howl k = __ // <- multiple words on this line.  You MUST use `f`.
    ...
    let ``35 Getting closure`` () =
        ...
        calculate 10 20 5 |> should equal (0, 10)  //calculate 10 20 5 |> should equal __
        calculate 0 600 250 |> should equal (-50, 550)  //calculate 0 600 250 |> should equal __
    ...
    let ``36 Using a value defined in an inner scope`` () =
        ...
        let g t =
            let result = ((t%2)+1) * 10
            fun x -> result - x
        g 5 8 |> should equal 12  //g 5 8 |> should equal __
        g 8 5 |> should equal  5     //g 8 5 |> should equal __
    ...
    let ``37 An operator is just a function in disguise`` () =
        let apply f x =
            f x 3
        apply (/) 27 |> should equal 9 //apply (/) 27 |> should equal __
        apply (*) 4 |> should equal 12   //apply (*) 4 |> should equal __
