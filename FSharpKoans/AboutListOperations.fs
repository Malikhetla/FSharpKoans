    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
             let rec  count xs iter = // write a function to find the length of a list
                 match xs with 
                 | [] -> iter
                 | _::rest -> count (rest) (iter+1)
             count xs 0
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
            let rec shift xs newLst =   // write a function to reverse a list here.
                match xs with 
                | [] ->  newLst
                | a::rest -> shift(rest) (a::newLst)
            shift xs []
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            let rec change xs mapped=  // write a function which adds 1 to each element
               match xs with 
               | [] -> mapped
               | a::rest -> change(rest) (a+1::mapped)
            List.rev(change xs [])
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        ...
        let map (xs : int list) : int list =
            let rec double xs num = 
               match xs with 
               |[] -> num
               |a:: rest -> double (rest) (a+a::num)
            List.rev(double xs [])
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let rec incrme1 f xs lst = // write a map which applies f to each element
              match xs with 
              | [] -> lst
              | a::rest -> incrme1 f  (rest) (f a::lst)
            List.rev(incrme1 f xs [])
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec findSpec f xs newList = // write a function which filters based on the specified criteria
               match xs with 
               | [] -> newList 
               | a::rest -> 
                   match f a with
                   |true -> findSpec f rest (a::newList)
                   |_ -> findSpec f rest newList
            List.rev(findSpec f xs [])
    ...
    let ``11 Fixed-function filtering, the hard way`` () =
        let filter (xs : int list) : int list =
            let rec findOdd xs anotherLst = // write a function to filter for odd elements only.
              match xs with 
              | [] -> anotherLst
              | a::rest -> 
                  match a % 2 = 1 with 
                  | true -> findOdd rest (a::anotherLst)
                  | _ -> findOdd rest anotherLst
            List.rev(findOdd xs [])
   ...
   // ... you can make a function to do that, right? ^_^.
    ...
    let ``12 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec speciFilter f xs newList = // write a function which filters based on the specified criteria
               match xs with 
               | [] -> newList
               | a::rest -> 
                   match f a with 
                   |true -> speciFilter f rest (a::newList)
                   | _ -> speciFilter f rest newList
            List.rev(speciFilter f xs [])
        filter (fun x -> x > 19) [9; 5; 23; 66; 4] |> should equal [23; 66]
        filter (fun x -> String.length x = 4) ["moo"; "woof"; "yip"; "nyan"; "meow"]
        |> should equal ["woof"; "nyan"; "meow"]
        filter (fun (a,b) -> a*b >= 14) [9,3; 4,2; 4,5] |> should equal [9,3; 4,5]
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            let rec addList n xs =           // write a function to do what's described above
              match xs with 
              | [] -> n
              | a::rest -> addList (n+a) rest
            addList initialState xs  
    ...
    let ``15 A fold which multiplies a list`` () =
        let fold initialState xs =
            let rec multList n xs =              // write a function to multiply the elements of a list
              match xs with 
              | [] -> n
              | a::rest -> multList (n*a) rest 
            multList initialState xs
    ...
    let ``16 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            let rec functionFold f lastVal xs  =  // write a function to do a fold.
              match xs with 
              | [] -> lastVal
              | a::rest -> functionFold f (f lastVal a) rest 
            functionFold f initialState xs
        fold (+) 0 [1;2;3;4] |> should equal 10
        ...
        //ASK ABOUT THIS!!!!//fold (fun state item -> state + float item + 0.5) 0.8 [1;3;5;7] |> should equal 18.8
    ...
    let ``18 exists: finding whether any matching item exists`` () =
        let exists (f : 'a -> bool) (xs : 'a list) : bool =
           let rec check f xs resul = // Does this: https://msdn.microsoft.com/en-us/library/ee370309.aspx
               match xs with 
               | [] -> resul
               | a::rest -> 
                     match f a with
    ...
    let ``19 partition: splitting a list based on a criterion`` () =
       let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
         // jfvnfjvfj
          ...
          let rec separate f xs aList bList = 
             match xs with 
             | [] -> (List.rev(aList), List.rev(bList))
             | x::rest -> 
                     match f x with 
                     | true -> separate (f) (rest) (x::aList) (bList) 
                     | false -> separate (f) (rest) (aList) (x::bList)
          separate f xs [] [] 
    ...
    let ``20 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec make  n f lst =  // Does this: https://msdn.microsoft.com/en-us/library/ee370497.aspx
               match n = 0 with 
               | true -> lst 
               | false -> make (n-1) f (f (n-1)::lst)
            make n f []
    ...
    let ``21 tryFind: find the first matching element, if any`` () =
        let tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
            let rec lookFor p xs =  // Does this: https://msdn.microsoft.com/en-us/library/ee353506.aspx
              match xs with 
              | [] -> None
              | a::rest -> 
                     match p a with 
                     | true -> Some a
                     | false -> lookFor p rest
            lookFor p xs 
    ...
    *)
    ...
    let ``24 mapi: like map, but passes along an item index as well`` () =
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
            ...
            let rec func f ind xs lst = 
              match xs with 
              | [] -> List.rev(lst)                    //(List.rev((f (ind) (a))::lst))
              | a::rest -> func f (ind+1) (rest) ((f (ind) (a))::lst)
            func f 0 xs []
