...
module ``04: Match expressions`` = 
    ...
    let ``01 Basic match expression`` () =
        match 8000 with
        | 8000 -> "Insufficient power-level" // __ -> "Insufficient power-level"
        ()
    ...
    let ``02 Match expressions are expressions, not statements`` () =
        let result =
            match 9001 with
            | 9001 ->  // FILL_ME__IN ->   // <-- use an identifier pattern here!
                match 9001 + 1000 with   //match __ + 1000 with    // <-- now use the identifier that you've bound
    ...
    let ``03 Shadowing in match expressions`` () =
        ...
        | 19 -> ()
        | y ->
            y |> should equal 213  // y |> should equal __
            x |> should equal 213  //x |> should equal__
        y |> should equal 19
